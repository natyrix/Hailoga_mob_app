...
class _EditCheckListState extends State<EditCheckList> {
  ...
  _updateChkList() async{
    ...
    CheckList chk = CheckList(
      ...
      status: status,
      dateAndTime: '${selectedDate.toString().split(' ')[0]} ${selectedTime.hour}:${selectedTime.minute}:00.0'
    );
    _apiResponse = await service.EditChk(chk);
    setState(() {
      _isLoading = false;
    });
  ...
  @override
  void initState() {
    status = widget.checkList.status;
    contentController.text = widget.checkList.content;
    orderNumberController.text = widget.checkList.orderNumber.toString();
    selectedDate = DateTime.parse(widget.checkList.dateAndTime);
    selectedTime = TimeOfDay.fromDateTime(DateTime.parse(widget.checkList.dateAndTime));
    timeHr = selectedTime.hour<10?"0${selectedTime.hour}":"${selectedTime.hour}";
    timeMin = selectedTime.minute<10?"0${selectedTime.minute}":"${selectedTime.minute}";
    super.initState();
  }
